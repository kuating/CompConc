...
class QSortConc{
	...
	public QSortConc(int[] v, int nthreads){
		...
		//inicializa fila;
		q = new PriorityQueue<Bounds>(tam, new BoundsComparator()); 
		
		q.add(new Bounds(0,tam-1));
		
		this.nullBounds = new Bounds(-1,-1); //coordenadas negativas
	}
	
	//Nao eh thread safe. Uso na main, quando nao tem threads rodando
	public void printV(){
	...
	public synchronized void endTask(int p,int r, int j){
		if(j==-1) count++; //caso de ser vetor de tam = 1
			...
			count++;
			q.add(new Bounds(p,j-1));
			q.add(new Bounds(j+1,r));
		}
		notifyAll();
	}
